...
def find_latest_palette_image():
    """Find the most recent palette image in the uploads directory"""
    # Try different possible paths for uploads directory
    possible_uploads_dirs = [
        '../uploads',  # When run from Recommandations directory
        'uploads',     # When run from main directory
        os.path.join(os.path.dirname(__file__), '../uploads'),  # Relative to script location
    ]
    
    uploads_dir = None
    for dir_path in possible_uploads_dirs:
        if os.path.exists(dir_path):
            uploads_dir = dir_path
            break
    
    if not uploads_dir:
        print(f"Warning: uploads directory not found in any of: {possible_uploads_dirs}")
        return '../uploads/1.png'  # fallback
    
    # Find all palette PNG files
    palette_files = glob.glob(os.path.join(uploads_dir, 'palette-*.png'))
    
    if not palette_files:
        print("Warning: No palette files found, using fallback")
        return '../uploads/1.png'  # fallback
    
    # Sort by modification time (most recent first)
    latest_file = max(palette_files, key=os.path.getmtime)
    print(f"Found latest palette file: {latest_file}")
    return latest_file

# Paths to the CSV files. Updated to use emotions generation data
script_dir = os.path.dirname(os.path.abspath(__file__))
ORIGINAL_DATA_PATH = os.path.join(script_dir, '../emotions_generation/interpretable_color_features_cleaned.csv')
RESAMPLED_DATA_PATH = os.path.join(script_dir, '../emotions_generation/resampled_emotions_data.csv')

# Number of recommendations to return
NUM_RECOMMENDATIONS = 10
# Number of color features (updated to 85)
NUM_FEATURES = 85

# Path to the image file to analyze
IMAGE_PATH = 'uploads/palette-1752491806898-94e7fe9f-6dd7-4cd3-84cf-7fcdd4a0493b.png'  # This will be replaced by the server
...
def load_emotion_models():
    """Load emotion prediction models"""
    models_dir = os.path.join(script_dir, '../emotions_generation')
    try:
        model = joblib.load(os.path.join(models_dir, 'final_emotion_model.pkl'))
        scaler = joblib.load(os.path.join(models_dir, 'final_scaler.pkl'))
        label_encoder = joblib.load(os.path.join(models_dir, 'final_label_encoder.pkl'))
        feature_selector = joblib.load(os.path.join(models_dir, 'final_feature_selector.pkl'))
        return model, scaler, label_encoder, feature_selector
    except Exception as e:
        print(f"Error loading emotion models: {e}")
        return None, None, None, None

def predict_emotion(color_features):
    """Predict emotion from color features"""
    model, scaler, label_encoder, feature_selector = load_emotion_models()
    ...
    try:
        # Scale features
        features_scaled = scaler.transform(color_features)
        ...
        # Fallback: simple emotion prediction based on color features
        return simple_emotion_prediction(color_features)
...
# --- Generate 25 Color Features from User Color Selection ---
...
def create_color_features_from_selection(user_colour_selection):
    ...
    # Color dominance features (24 features)
...
# Create 85 color features from user selection
user_color_features = create_color_features_from_selection(user_colour_selection)

# Predict emotion from color features
emotion_prediction = predict_emotion(pd.DataFrame([user_color_features]))
...
def recommend_paintings(user_selection, original_df, resampled_df, num_recs=10):
    ...
    # Get the 25 color features from the resampled data
...
def get_recommendations_from_candidates(candidate_df, original_df, num_needed):
    """
    Helper function to get recommendations from a set of candidate vectors
    """
    if candidate_df.empty or num_needed <= 0:
        return []
    
    # Sample candidates
    num_to_sample = min(num_needed, len(candidate_df))
    search_vectors_df = candidate_df.sample(n=num_to_sample, random_state=42)
    
    # Get feature vectors (85 color features)
    feature_cols = COLOR_FEATURES
    search_vectors = search_vectors_df[feature_cols].values
    original_vectors = original_df[feature_cols].values
    
    # Find nearest neighbors
    nn_model = NearestNeighbors(n_neighbors=min(5, len(original_df)), metric='cosine', algorithm='brute')
    nn_model.fit(original_vectors)
    distances, indices = nn_model.kneighbors(search_vectors)
    ...
    for i in range(indices.shape[1]):
        for j in range(indices.shape[0]):
            ...
            if len(recommended_indices) >= num_needed:
                break
        if len(recommended_indices) >= num_needed:
            break
    
    return original_df.iloc[recommended_indices]['url'].tolist()
...
if __name__ == "__main__":
    ...
    print(f"Loading resampled data from '{RESAMPLED_DATA_PATH}'...")
