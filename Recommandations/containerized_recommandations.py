...
def predict_emotion_via_api(color_features, api_url="http://emotion-api:8000"):
    ...
    try:
        ...
        print(f"Calling emotion API at {api_url}/predict with colors: {color_data}")
        ...
        if response.status_code == 200:
            ...
            if predictions:
                ...
                print(f"API returned dominant emotion: {dominant_emotion[0]} with confidence: {dominant_emotion[1]:.3f}")
                return {
                ...
                print("API returned no predictions, using fallback")
        ...
        print(f"Error calling emotion API: {e}")
        return simple_emotion_prediction(features_dict)

def simple_emotion_prediction(color_features):
    """Simple emotion prediction based on color features when API fails"""
    try:
...
# Import all the color extraction functions from the original script
# (keeping the same functions as in the original script)
...
# Import the rest of the functions from the original script
